...
const game = {
    ...
    setGameScoreButton() {
        const gameScoreButton = document.getElementById('gameScoreButton');
        const score = (this.snake.getBody().length - 1);
        gameScoreButton.textContent = 'Счет: ' + score;
    },

    tickHandler() {
        if ((!this.canMakeStep()) || (this.wall.isOnPoint(this.snake.getNextStepHeadPoint()))) return this.finish();
